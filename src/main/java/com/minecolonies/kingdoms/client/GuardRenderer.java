...
/**
 * Kingdoms-owned renderer for garrison guards (Phase 9): the citizen humanoid model with MineColonies' installed knight
 * citizen textures referenced at runtime (never copied), plus vanilla armour and held-item layers for their iron gear.
 * Falls back to the default MineColonies style and then to a vanilla skin, like settlement representatives.
 */
public final class GuardRenderer extends MobRenderer<SettlementGuardEntity, PlayerModel<SettlementGuardEntity>>
{
    private final PlayerModel<SettlementGuardEntity> citizenWide;
    private final PlayerModel<SettlementGuardEntity> citizenSlim;
    private final PlayerModel<SettlementGuardEntity> vanillaWide;
    private final PlayerModel<SettlementGuardEntity> vanillaSlim;

    public GuardRenderer(final EntityRendererProvider.Context context)
    {
    ...
    @Override
    public void render(final SettlementGuardEntity entity, final float yaw, final float partialTicks, final PoseStack pose,
    ...
    @Override
    public ResourceLocation getTextureLocation(final SettlementGuardEntity entity)
    {
        return appearance(entity).texture();
    }

    private Appearance appearance(final SettlementGuardEntity entity)
    {
    ...
    private record Appearance(ResourceLocation texture, PlayerModel<SettlementGuardEntity> model) {}
}
